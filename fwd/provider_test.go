...
func Test_InProcessProvider_Empty(t *testing.T) {
	p := NewInProcess(nil, nil)
	defer p.Shutdown()
	...
	if !errors.Is(err, ErrExhausted) {
		t.Fatalf("expected ErrExhausted for empty serials, got %v", err)
	}
}

func Test_InProcessProvider_Cancel(t *testing.T) {
	p := NewInProcess([]string{"DEADSERIAL1", "DEADSERIAL2"}, nil)
	defer p.Shutdown()
	...
	_, err := p.Acquire(ctx)
	if err == nil || (!errors.Is(err, context.Canceled) && !errors.Is(err, ErrExhausted)) {
		t.Fatalf("expected context.Canceled or ErrExhausted, got %v", err)
	}
}

func Test_InProcessProvider_Shutdown(t *testing.T) {
	p := NewInProcess([]string{"DEAD1", "DEAD2"}, nil)
	p.Shutdown()
