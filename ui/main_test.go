...
// Промпт обновы: стрелки двигают курсор между «обновить»/«позже»,
// enter на «позже» — дальше в меню (ветку установки тут не дёргаем).
func TestUpdatePromptArrows(t *testing.T) {
	chdirTemp(t)
	m := initialModel(nil)
	cfg.IsActivated = true
	m.upd = &update.Release{Version: "9.9", Tag: "v9.9"}
	m.state = stUpdate

	nm, _ := m.updateUpdatePrompt(tea.KeyMsg{Type: tea.KeyDown})
	m = nm.(model)
	if m.updCur != 1 {
		t.Fatalf("вниз: updCur = %d, want 1", m.updCur)
	}
	nm, _ = m.updateUpdatePrompt(tea.KeyMsg{Type: tea.KeyDown})
	m = nm.(model)
	if m.updCur != 0 {
		t.Fatalf("вниз с края: updCur = %d, want 0 (закольцован)", m.updCur)
	}
	nm, _ = m.updateUpdatePrompt(tea.KeyMsg{Type: tea.KeyUp})
	m = nm.(model)
	if m.updCur != 1 {
		t.Fatalf("вверх с края: updCur = %d, want 1 (закольцован)", m.updCur)
	}
	nm, _ = m.updateUpdatePrompt(tea.KeyMsg{Type: tea.KeyEnter})
	m = nm.(model)
	if m.state != stMenu || m.upd != nil {
		t.Fatalf("enter на «позже»: state=%v upd=%v, want stMenu/nil", m.state, m.upd)
	}
}
